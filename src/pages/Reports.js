  ...
  const fetchLiveAnalytics = async () => {
    try {
      ...
      const [trRes, perfRes, revRes] = await Promise.all([
        fetch(`${apiBase}/analytics/trends?${queries}`, { headers: authHeader }),
        fetch(`${apiBase}/analytics/performance`, { headers: authHeader }),
        fetch(`${apiBase}/analytics/revenue?period=${mapDateRangeToRevenue(dateRange)}`, { headers: authHeader }),
      ]);
